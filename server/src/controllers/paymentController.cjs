...
const createRazorpayOrder = async (req, res) => {
  try {
    ...
    if (amountInPaise < 100) {
      return res
        .status(400)
        .json({ message: "Amount must be at least ₹1." });
    }
...
const verifyRazorpayPayment = async (req, res) => {
  try {
    if (!process.env.RAZORPAY_KEY_SECRET) {
      return res
        .status(503)
        .json({ message: "Payment is not configured." });
    }
...
const refundRazorpayPayment = async (req, res) => {
  try {
    ...
    if (order.paymentResult?.status === "refunded") {
      return res
        .status(400)
    ...
    if (!order.isPaid || !order.paymentResult?.id) {
      return res.status(400).json({
        message: "Order is not paid via Razorpay.",
      });
    }
